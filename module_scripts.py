   ...
   [(store_script_param, ":sender_player_id", 1),
    ...
    (else_try), # section of events received by server from the clients
      ...
      (try_begin), # handle players requesting to attach a cart to themselves or a horse
		...
		(store_sub, ":actual_slot", ":from_slot", slot_scene_prop_inventory_item_0),
		(agent_get_item_slot, ":item_id", ":agent_id", ":actual_slot"),
		(assign, ":equipped_item", -1),
		(try_begin),
			(store_add, ":offset", "itm_black_helmet", 1),
			(is_between, ":item_id", "itm_tattered_headcloth", ":offset"),
		...
		(else_try),
			(store_add, ":offset", "itm_heraldic_mail_with_surcoat", 1),
			(is_between, ":item_id", "itm_ragged_shirt", ":offset"),
		...
		(else_try),
			(store_add, ":offset", "itm_black_greaves", 1),
			(is_between, ":item_id", "itm_tattered_wrapping_boots", ":offset"),
		...
		(else_try),
			(store_add, ":offset", "itm_gauntlets", 1),
			(is_between, ":item_id", "itm_leather_gloves", ":offset"),
		...
		(try_begin),
			(ge, ":equipped_item", "itm_tattered_headcloth"),
			(scene_prop_set_slot, ":container_id", ":from_slot", ":equipped_item"),
			(try_for_range, ":other_player_id", 1, ":max_players"), # loop over all players currently looking at the same inventory
			  (player_is_active, ":other_player_id"),
			  (neq, ":other_player_id", ":player_id"),
			  (player_slot_eq, ":other_player_id", slot_player_accessing_instance_id, ":container_id"),
			  (try_begin),
				(multiplayer_send_3_int_to_player, ":other_player_id", server_event_scene_prop_set_slot, ":instance_id", ":from_slot", ":equipped_item"),
			  (try_end),
			(try_end),
			(scene_prop_get_slot, ":prune_time", ":container_id", slot_scene_prop_prune_time),
			(try_begin), # if the inventory is stored in a spawned item, remove it if all items are taken out
			  ...
			  (scene_prop_get_slot, ":inventory_end", ":container_id", slot_scene_prop_inventory_count),
			  (val_add, ":inventory_end", 1),
			  
			  (get_max_players, ":max_players"),
			  (try_for_range, ":inventory_slot", slot_scene_prop_inventory_begin, ":inventory_end"),
				(scene_prop_get_slot, ":item_id", ":container_id", ":inventory_slot"),
				(ge, ":item_id", all_items_begin),
				(assign, ":inventory_end", -1),
			  (try_end),
			  (neq, ":inventory_end", -1),
			  (scene_prop_set_prune_time, ":container_id", 1),
			(else_try), # otherwise extend the time it will be removed if that is soon
			  (store_mission_timer_a, ":time"),
			  (val_add, ":time", 60),
			  (gt, ":time", ":prune_time"),
			  (scene_prop_set_prune_time, ":container_id", 60),
			  (scene_prop_set_slot, ":container_id", slot_scene_prop_prune_time, ":time"),
			(try_end),
		(try_end),
		(display_message, "@equipped"),
		#End
	  (else_try),
		#Fast inventory transfer
